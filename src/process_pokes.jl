...
function process_pokes(filepath::String)
    curr_data = CSV.read(filepath) |> DataFrame
    if !iscolumn(curr_data,:Poke)
        DataFrames.rename!(curr_data, propertynames(curr_data)[1] => :Poke) #change poke counter name
    end
    if iscolumn(curr_data,:delta)
    ...
    if iscolumn(curr_data,:ProbVec0)
        ...
        curr_data[!,:Protocol] = [r.Protocollo == 0 ? string(r.ProbVec0,"/",r.GamVec0) : string(r.ProbVec1,"/",r.GamVec1) for r in eachrow(curr_data)]
    ...
    curr_data[!,:Streak] = count_different(curr_data[:,:Side])
    ...
    combine(groupby(curr_data,:Streak)) do dd
        dd[:,:Poke_within_Streak] = count_different(dd[!,:Poke])
    ...
    curr_data[!,:Block] = count_different(curr_data[!,:Wall])
    curr_data[!,:Streak_within_Block] .= 0
    combine(groupby(curr_data,:Block)) do dd
        dd[:,:Streak_within_Block] = count_different(dd[!,:Side])
    end
